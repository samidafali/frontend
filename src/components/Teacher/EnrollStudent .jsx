...
const EnrollStudent = () => {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BACKEND_BASE_URL}/api/courses`, {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BACKEND_BASE_URL}/api/students`, { // Remplacez par votre API pour récupérer la liste des étudiants
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
      })
      .then((response) => {
  ...
  const handleEnrollUser = () => {
    ...
    axios
      .put(`${process.env.REACT_APP_BACKEND_BASE_URL}/api/courses/${selectedCourse}/enroll`, { userId: selectedStudent })
      .then((response) => {
