...
const AdminTeachers = () => {
  ...
  const fetchTeachers = async () => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/api/admin/teachers`, {
        headers: {
  ...
  const handleApproval = async (teacherId, isApproved) => {
      try {
          ...
          const token = localStorage.getItem('token');
          await axios.put(`${process.env.REACT_APP_BACKEND_BASE_URL}/api/admin/approve-teacher/${adminId}/${teacherId}`, {
              isApproved,
